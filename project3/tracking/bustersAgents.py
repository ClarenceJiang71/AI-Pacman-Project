...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = [beliefs for i,beliefs
                                            ...
                                            if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        distance_list = util.Counter()
        # first added all the maze distance of all existed ghosts which has the highest probability into a Counter
        for living_ghost_dist in livingGhostPositionDistributions:
            highest_prob_posi = living_ghost_dist.argMax()
            distance_list[highest_prob_posi] = self.distancer.getDistance(pacmanPosition, highest_prob_posi)
        # sort the Counter with the given method and get the last one in the list which should have the lowest distance
        sorted_key_list = distance_list.sortedKeys()
        min_prob_posi = sorted_key_list[-1]

        min_action = None
        min_distance = 100000000
        # after determining which ghost is the closest, we just simply iterate through each legal action to see
        # which action leads to a lowest maze distance
        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            distance = self.distancer.getDistance(successorPosition, min_prob_posi)
