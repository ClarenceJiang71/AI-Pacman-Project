...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        "*** YOUR CODE HERE ***"
        # print (type(emissionModel))
        # print (emissionModel)
        # print (self.beliefs)
        # print (type(pacmanPosition))
        # print (noisyDistance)
        ...
        # Directly assign the probability at the jail position to 1, so the ghost will absolutely
        # be sitting in the jail
        if noisyDistance is None:
            allPossible[self.getJailPosition()] = 1.0

        # Assign the trust weight to each conditional probability, so we can adjust how the probability
        # at each legal position changes. The new conditional probability can be accessed through emission
        # model, while the old trust factor can just be extracted from previous iteration through the instance
        # attribute, self.beliefs.
        for p in self.legalPositions:
            trueDistance = util.manhattanDistance(p, pacmanPosition)
            conditional_prob = emissionModel[trueDistance]
            trust_weight = self.beliefs[p]
            allPossible[p] = conditional_prob * trust_weight
    ...
    def elapseTime(self, gameState):
        ...
        current position (e.g.,t for DirectionalGhos).  However, this is not a problem,
        ...
        *** GORY DETAIL AHEAD ***
        ...
        "*** YOUR CODE HERE ***"
        allPossible = util.Counter()
        # The old position is exactly stored in the self.beliefs instance attribute, each key of the
        for old_posi, old_prob in self.beliefs.items():
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, old_posi))
            for newPos, prob_new in newPosDist.items():
                allPossible[newPos] += newPosDist[newPos]*self.beliefs[old_posi]
        allPossible.normalize()
        self.beliefs = allPossible
...
class ParticleFilter(InferenceModule):
    ...
    def __init__(self, ghostAgent, numParticles=300):
        ...
        #   create a list object which will be consistently used in the following methods, so
        #   I just make it as a class feature here.
        self.particle_list = []
    ...
    def initializeUniformly(self, gameState):
        ...
        legal_posi = self.legalPositions
        length_legal_action = len(legal_posi)
        times = num_particle/length_legal_action
        module = num_particle % length_legal_action
        ...
        for j in range(module):
            particle_list.append(legal_posi[j])
    ...
    def observe(self, observation, gameState):
        ...
        #   assign all the number of particles all to the jail position
        if noisyDistance is None:
            self.particle_list = []
            jail_position = self.getJailPosition()
            for i in range(self.numParticles):
                self.particle_list.append(jail_position)
        #   if that's not the case, it means we need to update the belief distribution based on observation
        else:
            emissionModel = busters.getObservationDistribution(noisyDistance)
            pacmanPosition = gameState.getPacmanPosition()
            allPossible = util.Counter()

            #   this line is a key line that deals with my previous bug, I keep getting timeout error because I
            #   directly call self.getBeliefDistribution() in the for-loop, so it costs me extra time in the for loop
            belief_dist = self.getBeliefDistribution()

            #   Do the same thing here, but we need to update through plus assign, since unlike question1, we are not
            #   going to store this allPossible Counter object as an instance attribute, but only as a tool to get
            #   sample in the following steps.

            for p in self.legalPositions:
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                conditional_prob = emissionModel[trueDistance]
                trust_weight = belief_dist[p]
                allPossible[p] += conditional_prob * trust_weight

            #   deal with the special case that all weights are zero, just break when iterating into a non-zero
            #   value, the boolean variable will be changed into False then.
            weight_zero = True
            ...
            #   if running into the all-zero case, then simply re-initialize and nothing else needs to be done
            #   if not running into that case, we need to generate sample from the belief distribution and
            #   update the new particle list.
            if not weight_zero:
                self.particle_list = []
                counter = 0
    ...
    def elapseTime(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        """
        The exactInference implementation here

        belief_dist = util.Counter()
        for old_posi, old_prob in self.beliefs.items():
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, old_posi))
            for newPos, prob_new in newPosDist.items():
                belief_dist[newPos] += newPosDist[newPos]*self.beliefs[old_posi]
        belief_dist.normalize()
        self.beliefs = belief_dist

        """
        for i in range(len(self.particle_list)):
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, self.particle_list[i]))
            self.particle_list[i] = util.sample(newPosDist)
    ...
    def getBeliefDistribution(self):
        ...
        "*** YOUR CODE HERE ***"
        belief_dist = util.Counter()
        #   Basically the idea is counting how many times a particular position occurs, since one
        #   occurrence exactly means one particle staying in that position.
        for particle in self.particle_list:
            belief_dist[particle] = belief_dist[particle] + 1
        belief_dist.normalize()
        return belief_dist
